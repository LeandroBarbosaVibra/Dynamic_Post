...
* Handles harmonic loadcases with repeated increment IDs (8, 8:1, 8:2, etc.)
...
Author: Leandro Barbosa"""
    ...
    # =========================================================================
    # FAST INCREMENT LOADING WITH HARMONIC SUPPORT (OPTIMIZED)
    # =========================================================================
    def load_increments_fast(self, t16_file):
        """
        Load increment data with FAST detection and harmonic loadcase support.
        
        OPTIMIZED: Instead of reading ALL increments, we only read 3:
        - First increment (to get starting ID)
        - Last increment (to get final ID)
        - Second-to-last increment (to detect if harmonic exists)
        
        If last == second-to-last, harmonic loadcase is detected.
        The harmonic start position is calculated mathematically.
        
        Example: 20 increments, first_id=0, last_id=10, second_last_id=10
        - Harmonic detected (last == second_last)
        - Regular increments: 0 to 10 (11 unique values)
        - Harmonic starts at position: start_index + (last_id - first_id + 1)
        - Labels: 0, 1, 2, ..., 10, 10:1, 10:2, ..., 10:N
        
        Returns a dictionary with:
        - 'indices': list of internal indices (0-based position in .t16)
        - 'ids': list of actual increment IDs (raw)
        - 'labels': list of UI labels (with :1, :2 suffixes for duplicates)
        ...
        try:
            ...
            self.set_progress_label("Detecting increment structure (fast mode)...", "blue")
            self.progress_bar['value'] = 30
            self.root.update_idletasks()
            
            # =====================================================================
            # OPTIMIZED: Read only 3 increments to detect harmonic loadcase
            # =====================================================================
            last_index = total_inc - 1
            second_last_index = total_inc - 2 if total_inc >= 2 else start_index
            
            # Read first increment
            self.set_progress_detail("Reading first increment...")
            post.moveto(start_index)
            first_id = post.increment
            
            self.progress_bar['value'] = 50
            self.root.update_idletasks()
            
            # Read last increment
            self.set_progress_detail("Reading last increment...")
            post.moveto(last_index)
            last_id = post.increment
            
            self.progress_bar['value'] = 70
            self.root.update_idletasks()
            
            # Read second-to-last increment (to detect harmonic)
            self.set_progress_detail("Checking for harmonic loadcase...")
            post.moveto(second_last_index)
            second_last_id = post.increment
            ...
            # =====================================================================
            # Detect harmonic loadcase: if last_id == second_last_id
            # =====================================================================
            has_harmonics = (last_id == second_last_id) and (last_index > second_last_index)
            
            # Build increment lists
            increment_indices = list(range(start_index, total_inc))
            increment_ids = []
            increment_labels = []
            label_to_index = {}
            
            if has_harmonics:
                # Calculate where harmonic starts
                # Regular increments go from first_id to last_id (inclusive)
                num_unique_increments = last_id - first_id + 1
                harmonic_start_position = num_unique_increments  # Position in our list (0-based)
                
                self.set_progress_detail(f"Harmonic detected at increment {last_id}...")
                
                # Build the lists
                for i, idx in enumerate(increment_indices):
                    if i < harmonic_start_position:
                        # Regular increment
                        inc_id = first_id + i
                        label = str(inc_id)
                    else:
                        # Harmonic increment
                        inc_id = last_id
                        harmonic_suffix = i - harmonic_start_position + 1
                        label = f"{last_id}:{harmonic_suffix}"
                    
                    increment_ids.append(inc_id)
                    increment_labels.append(label)
                    label_to_index[label] = idx
                
                num_harmonic_freqs = actual_count - harmonic_start_position
                self.set_progress_label(f"Loaded {actual_count} increments (harmonic detected)", "green")
                self.set_progress_detail(f"Increment {last_id} has {num_harmonic_freqs} harmonic frequencies")
            else:
                # No harmonic - sequential increments
                self.set_progress_detail("Building increment list...")
                ...
                self.set_progress_label(f"Loaded {actual_count} increments", "green")
                self.set_progress_detail(f"IDs: {first_id} to {last_id}")
            ...
            return {
                ...
                'harmonic_increment': last_id if has_harmonics else None
            }
...
def get_parameters(root, increment_data=None):
    ...
    if has_harmonics:
        tk.Label(file_info_frame, text="(Harmonic loadcase detected)", font=('Arial', 8, 'italic'), 
    ...
    help_text = """* Starting Increment: Select the increment to start the frequency sweep.
  - For harmonic loadcases, increments appear as: 8, 8:1, 8:2, etc.
  - The suffix (:1, :2) indicates repeated increments for different frequencies.
